(function (b) {
    var a = {
            ...
            if (!d.data("jqv") || d.data("jqv") == null) {
                ...
                b(document).on("click", ".formError", function () {
                    b(this).fadeOut(150, function () {
                        b(this).closest(".formError").remove()
                    })
                })
            }
            return this
        ...
        detach: function () {
            var d = this;
            var c = d.data("jqv");
            d.off(c.validationEventTrigger, "[" + c.validateAttribute + "*=validate]:not([type=checkbox]):not([type=radio]):not(.datepicker)", a._onFieldEvent);
            d.off("click", "[" + c.validateAttribute + "*=validate][type=checkbox],[" + c.validateAttribute + "*=validate][type=radio]", a._onFieldEvent);
            d.off(c.validationEventTrigger, "[" + c.validateAttribute + "*=validate][class*=datepicker]", a._onFieldEvent);
            d.off("submit", a._onSubmitEvent);
            d.removeData("jqv");
            ...
            d.removeData("jqv_submitButton");
            if (c.autoPositionUpdate) {
                b(window).off("resize", a.updatePromptsPosition)
            }
            return this
        },
        validate: function (g) {
        ...
        hide: function () {
            ...
            var d = f.data("jqv");
            if (!d) {
                d = a._saveOptions(f, d)
            }
            var e = (d && d.fadeDuration) ? d.fadeDuration : 0.3;
            var c;
            if (f.is("form") || f.hasClass("validationEngineContainer")) {
                c = "parentForm" + a._getClassName(b(f).attr("id"))
            } else {
                c = a._getClassName(b(f).attr("id")) + "formError"
            }
            b("." + c).fadeTo(e, 0, function () {
                b(this).closest(".formError").remove()
            });
            return this
        },
        hideAll: function () {
            ...
            b(".formError").fadeTo(e, 0, function () {
                b(this).closest(".formError").remove()
            });
            return this
        },
                ...
                if (b.inArray(p.attr("name"), o) < 0) {
                    ...
                    if (f && n == null) {
                        if (p.is(":hidden") && m.prettySelect) {
                            ...
                            if (p.data("jqv-prompt-at") instanceof jQuery) {
                                p = p.data("jqv-prompt-at")
                            } else {
                                if (p.data("jqv-prompt-at")) {
                                    p = b(p.data("jqv-prompt-at"))
                                }
                            }
                            n = p
                        }
            ...
            if (f) {
                if (m.scroll) {
                    ...
                    if (m.isOverflown) {
                        ...
                        l += d + g - 5;
                        var k = b(m.overflownDIV).filter(":not(:animated)");
                        k.animate({scrollTop: l}, 1100, function () {
                            if (m.focusFirstField) {
                                n.focus()
                            }
                        })
                    } else {
                        b("html, body").animate({scrollTop: l}, 1100, function () {
                            if (m.focusFirstField) {
                                n.focus()
                            }
                        });
        ...
        _validateField: function (d, l, s) {
            ...
            if (d.hasClass(l.ignoreFieldsWithClass)) {
                return false
            }
            ...
            for (var x = 0, n = 0; x < r.length; x++) {
                ...
                switch (r[x]) {
                    ...
                    case"funcCallRequired":
                        g = a._getErrorMessage(e, d, r[x], r, x, l, a._funcCallRequired);
                        if (g !== undefined) {
                            t = true
                        }
                        break;
                ...
                if (x == 0 && v.indexOf("funcCallRequired") == 0 && g !== undefined) {
                    if (h != "") {
                        h += "<br/>"
                    }
                    h += g;
            ...
            if (!t && !(d.val()) && d.val().length < 1 && b.inArray("equals", r) < 0) {
                l.isError = false
            }
            var p = d.prop("type");
            var c = d.data("promptPosition") || l.promptPosition;
            if ((p == "radio" || p == "checkbox") && e.find("input[name='" + j + "']").length > 1) {
                if (c === "inline") {
                    d = b(e.find("input[name='" + j + "'][type!=hidden]:last"))
                } else {
                    d = b(e.find("input[name='" + j + "'][type!=hidden]:first"))
                }
                l.showArrow = l.showArrowOnRadioAndCheckbox
            }
        ...
        _getErrorMessage: function (c, n, l, p, g, q, m) {
            ...
            if (l === "custom" || l === "funcCall" || l === "funcCallRequired") {
                var o = p[j + 1];
                l = l + "[" + o + "]";
                delete (p[j])
            }
        ...
        _required: function (h, k, f, m, g) {
            switch (h.prop("type")) {
                ...
                case"checkbox":
                    ...
                    var c = h.attr("name");
                    if (d.find("input[name='" + c + "']:checked").length == 0) {
                        if (d.find("input[name='" + c + "']:visible").length == 1) {
        ...
        _funcCallRequired: function (e, f, d, c) {
            return a._funcCall(e, f, d, c)
        },
        ...
        _maxCheckbox: function (h, j, k, g, f) {
            var d = k[g + 1];
            var e = j.attr("name");
            var c = h.find("input[name='" + e + "']:checked").length;
            if (c > d) {
                f.showArrow = false;
        ...
        _showPrompt: function (i, g, h, f, e, d) {
            if (i.data("jqv-prompt-at") instanceof jQuery) {
                i = i.data("jqv-prompt-at")
            } else {
                if (i.data("jqv-prompt-at")) {
                    i = b(i.data("jqv-prompt-at"))
                }
            }
        ...
        _getPrompt: function (e) {
            ...
            var c = b("." + a._escapeExpression(d) + ".parentForm" + a._getClassName(f))[0];
            if (c) {
                return b(c)
            }
        },
